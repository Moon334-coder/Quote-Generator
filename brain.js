...
function ChangeQuote(){
    middle = start;
    let random = Math.floor(Math.random()*172);
    start = random;
    quote.innerHTML = Quotes[random].quote;
    author.innerHTML = Quotes[random].author ;
    let a = Quotes[random].topics.length;
    middle1 = start1;
    let b = Math.floor(Math.random()*a);
    start1 = b;
    type.innerHTML = Quotes[random].topics[b];
    previous.style.visibility = 'visible';
}
function PreviousQuote(){
    quote.innerHTML = Quotes[middle].quote ;
    author.innerHTML = Quotes[middle].author ;
    type.innerHTML = Quotes[middle].topics[start1];
    previous.style.visibility = 'hidden';
}
button.addEventListener('click',ChangeQuote);
previous.addEventListener('click',PreviousQuote);
